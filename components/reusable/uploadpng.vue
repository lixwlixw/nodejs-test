...
<!--<template>-->
		<!--<span class="input-group-btn">-->
			<!--<input type="text" class="form-control" type="text" style="width: 400px;display: none" placeholder="" readonly disabled>-->
             <!--&lt;!&ndash;<div style="width: 150px;height: 100px;float: left;border: 1px solid #cacaca;margin-right: 20px"></div>&ndash;&gt;-->
            <!--<div id="localImag"><img id="preview" src=""style="display: block; width: 150px; height: 100px;"></div>-->
			<!--<slot></slot>-->
		<!--</span>-->
    <!--<div style="float: left">-->
        <!--<button style="width:100px" class="btn btn-default" @click="subupload" id="test" type="submit" >文件上传</button>-->
        <!--<div>照片大小500K以内，支持PNG/JPG格式</div>-->
        <!--<div v-if="status"><strong v-text="msg"></strong></div>-->
        <!--<input type="file" id="doc" style="display:none" accept="{{ accept }}" v-on:click="setImagePreview" v-on:change="fileInputChange"  multiple="{{ multiple }}">-->
    <!--</div>-->
...
<!--</template>-->
<template>
		<span class="input-group-btn" id="localImag">
			<input type="text" id="preview" class="form-control" type="text" style="width: 380px" placeholder="" readonly disabled>
			<button style="width:100px;height: 40px;border-radius: 5px" class="btn btn-default" @click="subupload" id="test" type="submit">文件上传</button>
    		<input type="file" id="doc" style="display: none" accept="{{ accept }}" v-on:click="fileInputClick" v-on:change="fileInputChange" multiple="{{ multiple }}">
    		<slot></slot>
		</span>

    <p style="display:inline-block ; position: absolute;right: 95px;top: 45px;width: 130px;text-align:center" v-if="status"><strong v-text="msg"></strong></p>
</template>
<script>
    export default {
        data () {
