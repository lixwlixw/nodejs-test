...
function repoListFun(thisJson) {
    ...
    var listFunJson = {
        ...
        appendHtml: function () {
           ...
           var thisreponame = '';
           var repoComment;
            var ch_thisreponame='';//中文repo_item名称
            var ch_itemname='';//中文item名称
            var ch_reponame='';//中文item名称
            for (var i = 0; i < itemarr.length; i++) {
                ...
                if(thisJson.selOritem.isitem == 'repositories'){
                    ...
                    datalabel = '<span class=" '+this.judgeLabel(repoComment.data.label.sys.supply_style).vvclass+'">' + this.judgeLabel(repoComment.data.label.sys.supply_style).labelV + '</span>';
                    ...
                    repoComment = this.getItemComment(reponame,itemname);
                    thisrepcomment = repoComment.data.comment;
                    repoComment_reponeme = this.getRepoComment(reponame);

                    //chinese repo name
                    console.log('repoComment222:',repoComment_reponeme);

                    // ch_reponame=repoComment

                    //chinese item name
                    console.log('repoComment:',repoComment_reponeme)
                    ch_itemname=repoComment.data.ch_itemname;
                    ch_reponame=repoComment_reponeme.data.ch_repname;

                    var ch_thisreponame = ch_reponame+'/'+ch_itemname;


                    // label sys custom标签添加
                    var select_labels=repoComment.data.label.sys.select_labels;
                    if(select_labels!=''&&select_labels!=null&&select_labels!='null'){
                        datalabel_custom= '<span class="custom">' + select_labels + '</span>';
                    }
                    var select_labels_owner=repoComment.data.label.owner;
                    if(select_labels_owner!=undefined){
                        if(typeof(select_labels_owner)!=undefined&&select_labels_owner.length!=undefined){
                            for(var i=0;i<select_labels_owner.length;i++){
                                datalabel_admin+= '<span class="admin">' + select_labels_owner[i] + '</span>';
                            }
                        }
                    }
                    ...
                    datalabel = '<span class="'+this.judgeLabel(repoComment.data.label.sys.supply_style).vvclass+'">' + this.judgeLabel(repoComment.data.label.sys.supply_style).labelV + '</span>'+datalabel_custom;
                ...
                var html = '<div class="listBox">' +
                    '<div class="likeNumber">' +
                        '<img src="/img/indexNew/selects/like.png" alt="">' +
                        '<p>'+thisstarnum+'</p>'+
                    '</div>'+
                    '<div class="listLeftBox">' +
                        '<div class="listNameTitle chTitle">' +
                            '<a target="_blank" title="'+isitemdet+'/'+searchrepname+'" href="/'+isitemdet+'/'+searchrepname+'">'+ch_thisreponame+'</a>' +
                            thispricestate+
                        '</div>' +
                        '<div class="listNameTitle enTitle">' +
                        ...
                        '<div class="listbtIconWrop">' +
                            ...
                            '<div class="listTime">' +
                                '<img title="" datapalecement="top" data-toggle="tooltip" src="/img/indexNew/selects/images_19.png" onmouseover=this.src="/img/indexNew/selects/images_21.png" onmouseout=this.src="/img/indexNew/selects/images_19.png"   class="'+isdataOwnerIcon+'" data-original-title="更新时间">' +
                            ...
                            '</div>'+
                            '<div class="listTag">' +
                                '<img title="" datapalecement="top" data-toggle="tooltip" src="/img/indexNew/selects/images_24.png" onmouseover=this.src="/img/indexNew/selects/images_27.png" onmouseout=this.src="/img/indexNew/selects/images_24.png"   class="tagsIcon" data-original-title="'+istagsname+'">' +
                                '<span>' + istagsnum + '</span>' +
                            '</div>'+
                            '</div>' +
                        '</div>' +
